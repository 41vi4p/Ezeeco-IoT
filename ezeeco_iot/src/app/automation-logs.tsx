...
export default function AutomationLogsScreen() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const q = query(collection(firestore, 'automationLogs'), where('userId', '==', user.id), orderBy('timestamp', 'desc'), limit(50));
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <LinearGradient colors={headerGrad} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[styles.header, { paddingTop: insets.top + 8 }]}>
        <View style={styles.headerRow}>
          ...
          <Text style={styles.headerTitle}>Automation Logs</Text>
        </View>
      </LinearGradient>
      ...
      {loading ? (
        ...
        <View style={styles.empty}>
          ...
          <Text style={[styles.emptyTitle, { color: theme.text }]}>No automation logs</Text>
          <Text style={[styles.emptyText, { color: theme.textSecondary }]}>Automation activity will appear here</Text>
        </View>
      ) : (
        <FlatList
          data={logs}
          ...
          renderItem={({ item }) => (
            <View style={[styles.logItem, { backgroundColor: isDark ? '#333333' : '#FFFFFF', borderColor: theme.cardBorder }]}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={[styles.logTitle, { color: theme.text }]}>{item.automationName || 'Automation triggered'}</Text>
                <Text style={[styles.logAction, { color: theme.textSecondary }]}>{item.action || 'Action executed'}</Text>
                <Text style={[styles.logTime, { color: theme.textSecondary }]}>{formatTime(item.timestamp)}</Text>
              </View>
              <View style={[styles.statusBadge, { backgroundColor: item.success ? '#D1FAE5' : '#FEE2E2' }]}>
                <Text style={[styles.statusText, { color: item.success ? '#059669' : '#DC2626' }]}>{item.success ? 'OK' : 'Failed'}</Text>
              </View>
            </View>
          )}
        />
...
const styles = StyleSheet.create({
  ...
  logAction: { fontSize: 12, marginBottom: 2 },
  logTime: { fontSize: 11 },
